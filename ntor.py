    ...
    if ntorSecretKey:
        try:
            ...
            if base64:
                # .rstrip('\n').rstrip('==')
                # print(ntorPublicKey)
                ntorPublicKey = binascii.b2a_base64(bytearray(str(ntorPublicKey), 'Ascii'))
                #print(ntorPublicKey)
                ntorPublicKey = ntorPublicKey.decode('ASCII').rstrip('\n').rstrip('==')
        except Exception as error:
            raise NTORPublicKeyError("Error retrieving the NTOR public key: %s" % error)
        else:
